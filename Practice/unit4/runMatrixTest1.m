function runMatrixTest(a)
    % runMatrixTest: Test the readMatrixCCW function with a given matrix.
    % SYNTAX: runMatrixTest(a)
    % INPUT: a = an nxm matrix to be tested
    
    % Call the solution function
    b = readMatrixCCW(a);
    
    % Calculate the expected output manually for comparison
    bExpected = getExpectedOutput(a);
...
function expectedOutput = getExpectedOutput(a)
    % This function calculates the expected output for different matrix sizes.
    % You can add logic here to handle different shapes of matrices.
    
    % Assuming a fixed expected output for the sample provided.
    if isequal(a, reshape(1:30, 5, 6))
        expectedOutput = [1, 6, 11, 16, 21, 26, 27, 28, 29, 30, 25, 20, 15, 10, 5, 4, 3, 2, 7, 12, 17, 22, 23, 24, 19, 14, 9, 8, 13, 18];
    else
        % For other matrices, calculate the expected output here or leave it as an empty array.
        expectedOutput = [];
    end
end
